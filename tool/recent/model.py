...
def get_recent_users(region_id: int):
    ...
    try:
        ...
        cur.execute(
            ...
            f"FROM {MAIN_DB}.rececnt WHERE region_id = %s;",
            [region_id]
        )
        rows = cur.fetchall()
        for row in rows:
            data.append([row['account_id'], row['region_id']])
...
def get_user_recent(region_id: int, account_id: int):
    """获取用户的"""
    pool = DatabaseConnection.get_pool()
    conn = pool.connection()
    cur = None
    try:
        ...
        cur.execute(
            ...
            "UNIX_TIMESTAMP(i.last_battle_at) AS last_update_time, r.recent_class, "
            "UNIX_TIMESTAMP(r.last_query_at) AS last_query_time, UNIX_TIMESTAMP(r.last_update_at) AS recent_update_time "
            f"FROM {MAIN_DB}.user_basic as b LEFT JOIN {MAIN_DB}.user_info ON i.account_id = b.account_id "
            f"LEFT JOIN {MAIN_DB}.recent ON r.region_id = b.region_id AND r.account_id = b.account_id "
        ...
        data['info'] = {
            ...
            'last_update_time': row['last_battle_time']
        }
        data['recent'] = {
            'recent_class': row['recent_class'],
            'last_qurey_time': row['last_query_time'],
...
def update_user_recent(region_id: int, account_id: int, recent_class: int = None, last_update_time: int = None):
    ...
    try:
        ...
        if recent_class:
            cur.execute(
                f"UPDATE {MAIN_DB}.rececnt SET recent_class = %s "
                "WHERE region_id = %s AND account_id = %s;",
                [recent_class, region_id, account_id]
            )
        if last_update_time:
            cur.execute(
                f"UPDATE {MAIN_DB}.rececnt SET last_update_at = FROM_UNIXTIME(%s) "
                "WHERE region_id = %s AND account_id = %s;",
