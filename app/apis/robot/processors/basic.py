...
def process_signature_data(
    ...
    for ship_id in ship_ids:
        ship_data = temp_data.get(ship_id)[battle_type]
        account_data = [
    ...
    if overall_data['battles_count'] == 0:
        ...
        if not algo_type:
            pro_data['personal_rating'] = -2
            pro_data['win_rate_color'] = ColorUtils.get_rating_color(0,-1)
            pro_data['avg_damage_color'] = ColorUtils.get_rating_color(1,-2)
            pro_data['avg_frags_color'] = ColorUtils.get_rating_color(2,-2)
            pro_data['rating_color'] = ColorUtils.get_rating_color(3,-2)
        elif overall_data['value_battles_count'] != 0:
            pro_data['personal_rating'] = int(overall_data['personal_rating']/overall_data['value_battles_count'])
            ...
            pro_data['rating_color'] = ColorUtils.get_rating_color(3,overall_data['personal_rating']/overall_data['value_battles_count'])
        else:
            pro_data['personal_rating'] = -1
            pro_data['win_rate_color'] = ColorUtils.get_rating_color(0,-1)
            pro_data['avg_damage_color'] = ColorUtils.get_rating_color(1,-1)
            pro_data['avg_frags_color'] = ColorUtils.get_rating_color(2,-1)
            pro_data['rating_color'] = ColorUtils.get_rating_color(3,-1)
        ...
        pro_data['rating_color'] = "#{:02X}{:02X}{:02X}".format(*pro_data['rating_color'])
    ...
    pro_data['personal_rating'] = '{:,}'.format(pro_data['personal_rating']).replace(',', ' ')
